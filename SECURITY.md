...
Security fixes target the latest Murmur release on the Sparkle update feed.
Older builds are not patched separately — update in-app when a fix ships.
...
on this repository (**Security → Report a vulnerability**). That keeps the
...
- macOS version
- Steps to reproduce
- Impact (for example: key leakage, unexpected Accessibility use, injection into the wrong field)
